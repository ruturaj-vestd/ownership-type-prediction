...
def test_group_site_vs_operating_company_site_adds_flags_not_review_label():
    ...
    assert out.ownership_type in {"Other / Special Structures", "Diverse", "Listed Parent", "Private Equity", "Family", "Individual(s)"}
...
def test_wrong_legal_entity_fuzzy_choice_still_classifies_not_needs_review():
    web = make_web(legal_entity="Acme Maybe Ltd")
    ch = make_ch([])
    out = run_staged_decision("Acme", web, ch)
    assert out.ownership_type == "Other / Special Structures"


def test_corporate_psc_not_individual_label():
    extracted = OwnershipEvidenceResult(ownership_facts=OwnershipFacts(psc_type="corporate-entity-person-with-significant-control", psc_name="BidCo Ltd", control_band="75-100%", private_equity_signal=True))
...
def test_mixed_public_private_conflict_maps_to_special_structures_not_review():
    ...
    assert label in {"Listed Parent", "Other / Special Structures", "Private Equity"}
    assert needs_review is False


def test_conflicting_psc_api_and_pdf_not_needs_review_label():
    company = {
    ...
    assert label == "Other / Special Structures"
